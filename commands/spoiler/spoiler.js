...
module.exports = class SpoilerCommand extends Command {
	...
	printSpoilerTopic(message) {
		...
		message.say(`The current spoiler channel topic is ${topic}, set by ${sender.toString()}!`);
	}
	...
	setSpoilerTopic(message, topic) {
		...
		message.say(`The spoiler channel topic has been successfully updated to ${topic} ${sender.toString()}!`);
		if (message.channel.name !== config.spoiler.spoilerChannel) {
			spoilerChannel.send(`Topic updated to ${topic} by ${sender.toString()}!`);
		}
	}
}
